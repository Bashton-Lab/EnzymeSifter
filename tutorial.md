...
The file extension (.pdb) and a SEQRES records within it are required to Stage 2. Following submission of our 122 sequences to AlphaFold server, the jobs were named as the headers of the sequences (with a - instead of the .). Gene numbers were already removed from the headers of the filtered sequences as no duplicates remained in the filtered dataset. The output model_0 CIF file from each predicted structure was converted to the PDB format using GEMMI, without modifying the coordinates. Predicted structures in PDB format are available in pdbs.tar.gz within the Zenodo deposit [https://doi.org/10.5281/zenodo.20237962](https://doi.org/10.5281/zenodo.20237962). Original AlphaFold Server output is in `alphafold_predictions.tar.gz` in the same deposit. For the sake of testing the pipeline, the `pdbs.tar.gz` was downloaded and extracted with::
